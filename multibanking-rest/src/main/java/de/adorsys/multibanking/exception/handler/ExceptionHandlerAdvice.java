...
import de.adorsys.multibanking.exception.domain.Error;
...
public class ExceptionHandlerAdvice extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleExceptionInternal(Exception ex, @Nullable Object body, HttpHeaders headers,
		...
		handleLogging(ex);
		if(body==null)body = Error.builder().status(status).exception(ex).build();
		if(headers==null)headers = new HttpHeaders();
		return new ResponseEntity<>(body, headers, status);
	}

    private ResponseEntity<Object> handleInternal(Exception ex, Error error, HttpHeaders headers,
			HttpStatus status, WebRequest request) {
        ...
        if (error == null)
        	error = Error.builder().status(status).exception(ex).build();
    ...
    protected ResponseEntity<Object> handleErrorMessageException(ErrorKeyException ex, WebRequest request) {
        Error error = Error.builder().status(ex.getHttpStatus()).exception(ex).text(ex.getErrorKey()).moreInfo(ex.getMoreInfo()).build();
    ...
    public ResponseEntity<Object> handleAccessDeniedException(AccessDeniedException e, WebRequest request) {
        ...
        Error error = Error.builder().status(HttpStatus.UNAUTHORIZED).text(ErrorConstants.ERR_ACCESS_DENIED).exception(e).build();
    ...
    public ResponseEntity<Object> handleException(ParametrizedMessageException e, WebRequest request) {
        ...
        if (responseStatus != null) {
        	error = Error.builder().status(responseStatus.value()).text(responseStatus.reason()).params(e.getParamsMap()).exception(e).build();
        } else {
        	error = Error.builder().status(HttpStatus.BAD_REQUEST).text(e.getMessage()).params(e.getParamsMap()).exception(e).build();
        }
    	return handleExceptionInternal(e, error, null, error.getStatus(), request);
    }
    ...
    public ResponseEntity<Object> handleConstraintViolationException(ConstraintViolationException ex, WebRequest request) {
    	Error error = Error.builder().exception(ex).build();
    ...
    protected ResponseEntity<Object> handleMissingServletRequestParameter(MissingServletRequestParameterException ex, HttpHeaders headers,
            HttpStatus status, WebRequest request) {
    	Error error = Error.builder().status(HttpStatus.BAD_REQUEST)
    ...
    protected ResponseEntity<Object> handleHttpMediaTypeNotSupported(HttpMediaTypeNotSupportedException ex, HttpHeaders headers,
        ...
        Error error = Error.builder().status(HttpStatus.UNSUPPORTED_MEDIA_TYPE)
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
    	...
    	Error error = Error.builder().status(HttpStatus.BAD_REQUEST).exception(ex).build();
