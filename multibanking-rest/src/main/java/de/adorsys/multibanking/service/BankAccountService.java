        ...
        bankAccounts.forEach(source -> {
			...
			if (idToBeUsed == null) {
				...
				if (idToBeUsed == null) {
					...
					log.debug("PATCH 3.0.31 id is uuid, rather than iban or account number: " + idToBeUsed);
				} else {
					log.debug("PATCH 3.0.31 id is account number, rather than iban: " + idToBeUsed);
				}
			}
